  ...
  <header class="sticky top-0 z-40 bg-white/70 backdrop-blur border-b border-slate-100">
    <div class="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between gap-6">
      <div class="flex items-center gap-3">
        <a href="{{ route('catalog.index') }}" class="text-2xl font-semibold tracking-tight text-gray-900">Peter Market</a>
        <span class="hidden sm:inline-flex text-sm text-gray-500">Compras ágiles y seguras</span>
      </div>

      <nav class="flex items-center gap-3 md:gap-6">
        <a href="{{ route('catalog.index') }}" class="nav-link {{ request()->routeIs('catalog.*') ? 'active' : '' }}">Productos</a>
        <a href="{{ route('about') }}" class="nav-link {{ request()->routeIs('about') ? 'active' : '' }}">Nosotros</a>

        @auth
          <div class="hidden sm:flex items-center gap-4">
            <a href="{{ route('account.dashboard') }}" class="nav-link {{ request()->routeIs('account.*') ? 'active' : '' }}">Mi cuenta</a>
        ...
        @else
          <div class="flex items-center gap-3">
            ...
            <a href="{{ route('admin.login') }}" class="nav-link">Acceso admin</a>
          </div>
        @endauth

        <a href="{{ route('cart.index') }}" class="relative inline-flex items-center justify-center text-gray-700 hover:text-blue-700 transition" aria-label="Carrito">
